  ...
  const cag_option *option, char **c)
{
  ...
  if (option->value_name != NULL) {
    if (**c == '=') {
      ...
      // parameter for this option is missing.
      if (context->argc > context->index + 1) {
        ...
        *c = context->argv[context->index];
        ...
        context->identifier = CAG_ERROR_MISSING_VALUE;
...
bool cag_option_fetch(cag_option_context *context)
{
  ...
  // Reset our identifier to the unknown-option identifier. The value is set to
  // NULL, to make sure we are not carrying the parameter from the previous
  // option to this one.
  context->identifier = CAG_ERROR_UNKNOWN_OPTION;
  context->value = NULL;
  context->error_index = -1;
  context->error_letter = 0;
  ...
  cag_printer printer, void *printer_ctx)
{
  ...
  if (context->identifier == CAG_ERROR_MISSING_VALUE) {
    printer(printer_ctx, "Option '%s' requires a value.\n",
