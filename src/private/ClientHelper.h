#ifndef NVFLEXUTILS_H
#define NVFLEXUTILS_H
...
#define NVFLEX_IMPLEMENT_OBJECT_REFERENCE() \
...
template <typename T, std::enable_if_t<std::is_base_of_v<NvFlexObject, T>, int> = 0>
inline void SafeRelease(T*& ptr) {
    if (ptr) {
        ptr->release();
        ptr = nullptr;
