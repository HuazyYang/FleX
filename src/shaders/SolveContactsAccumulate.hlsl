...
RWStructuredBuffer<float4> deltas : register(u1);

[numthreads(256, 1, 1)]
void SolveContactsAccumulate(uint idx : SV_DispatchThreadID) {
    if (int(idx) < gParams.kNumParticles) {
        ...
        if (count != 0) {
            ...
            for (int contact = lastContact; contact >= 0; --contact) {
                ...
                if (adhesionEnabled)
                    correctionSum.xyz = correctionSum.xyz - (separation * gParams.kAdhesion) * plane.xyz;

                [branch] if (penetration < 0.0) {
                    float3 normalCorrection = correctionSum.xyz - penetration * plane.xyz;
                    correctionSum.w += 1.0;
                    ...
                    correctionSum.xyz = frictionEnabled ? frictionCorrection : normalCorrection;
                }
            }

            if (correctionSum.w > 0.0)
                deltas[idx] = deltas[idx] + correctionSum;
        }
    }
}
