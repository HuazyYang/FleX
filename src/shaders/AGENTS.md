...
### The compiler is the next lever, and it is FXC 6.3.9600.16384

Every shipped blob carries `Generated by Microsoft (R) HLSL Shader Compiler
6.3.9600.16384` — the Windows 8.1 SDK RTM compiler. All work here uses FXC
10.1 from the 10.0.19041.0 SDK. Every difference still outstanding on the eight
`SolveShapes` sources is a codegen choice that differs between those two
versions: scalar chains against two- and three-component packing, a negate
modifier against a multiply by a `+/-1.0` literal, `dcl_temps` 52 against 51.
None is expressible from HLSL.

Compiling the recovered sources with 6.3.9600 would remove that entire class at
once, and would turn the remaining question from "is this residual a defect?"
into "does the source reproduce the blob?", which is answerable. It is the only
lever left that has not been pulled.

It is not installed here: `C:\Program Files (x86)\Windows Kits\8.1` exists but
contains only `References\...\Windows.winmd`, no `bin`. Obtaining it means the
Windows 8.1 SDK, which puts `fxc.exe` at `Windows Kits\8.1\bin\x64\fxc.exe`.
`fxc.exe` resolves `d3dcompiler_47.dll` through the standard search order, so
the SDK's own copy must sit beside it -- the `C:\Windows\System32` copy is a
much later build and will silently give 10.x-era codegen, defeating the point.
Verify with `fxc.exe` printing 6.3.9600.16384 in the header of any `-Fc`
listing before trusting a single result from it.
...
What would promote them: compile the sources with FXC 6.3.9600.16384, the
compiler that produced the shipped blobs. Every outstanding difference is a
codegen choice that differs between 6.3 and 10.1 and none is reachable from
HLSL, so this is the one lever that can still move the result — see [The
compiler is the next lever](#the-compiler-is-the-next-lever-and-it-is-fxc-63960016384).
Short of that, the remaining work is to show that no source formulation makes
10.1 pack components the way 6.3 did, which is a much weaker result.
